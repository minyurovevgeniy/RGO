<?php
  ...
  $backURL='<a href="./changePhoto.php">Попробовать ещё раз</a>';
  ...
  */
  ...
  $photoComment=iconv("utf-8","cp1251",$_POST['comment']);
  ...
  */
  ...
  $stmt=$pdo->prepare("UPDATE `photos` SET `photo_public_link`=?, `photo_src`=?, `photo_comment`=? WHERE `photo_id`=?");
  $stmt->execute(array("http://rgo.xn--100-5cdnry0bhchmgqi5d.xn--p1ai/".$newFilenameToDatabase,$newFilename,$photoComment,$photo_id));
  include("./php/disconnect.php");

  echo '<a href="./changePhoto.php">Изменить ещё одну фотографию</a>';
  ?>
