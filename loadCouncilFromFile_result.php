...
//	include("./php/loadData.php");
	...
	$backUrl='<a href="loadCouncilFromFile.php">Попробуйте ещё раз</a><br>';

	$fileName=$_FILES['council_file']['tmp_name'];

	if ($_FILES['council_file']['size']<=0)
	{
		echo $backUrl;
		die("Файл пустой");
	}
	...
	{
		$stmt=$pdo->prepare("TRUNCATE `council_of_regional_department`");
	...
	for($rowNumber=$MIN_ROW;$rowNumber<=$MAX_ROW;$rowNumber++)
	{
		...
		$stmt=$pdo->prepare("SELECT COUNT(*) FROM `council_of_regional_department` WHERE `council_date`=? AND `council_time`=? AND `council_end_time`=? AND council_description=?");
		$stmt->execute(array($dateToWrite,$startTimeToWrite,$endTimeToWrite,$description));
		$row=$stmt->fetch(PDO::FETCH_LAZY);
		if ($row['COUNT(*)']<=0)
		{
			$stmt=$pdo->prepare("INSERT INTO `council_of_regional_department` SET `council_date`=?, `council_time`=?, `council_end_time`=?, `council_place`=?, council_description=?");
