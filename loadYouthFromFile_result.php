...
//	include("./php/loadData.php");
	...
	$backUrl='<a href="loadYouthFromFile.php">Попробуйте ещё раз</a><br>';

	$fileName=$_FILES['youth_file']['tmp_name'];

	if ($_FILES['youth_file']['size']<=0)
	{
		echo $backUrl;
		die("Файл пустой");
	}
	...
	{
		$stmt=$pdo->prepare("TRUNCATE `youth_club`");
	...
	for($rowNumber=$MIN_ROW;$rowNumber<=$MAX_ROW;$rowNumber++)
	{
		...
		$stmt=$pdo->prepare("SELECT COUNT(*) FROM `youth_club` WHERE `youth_date`=? AND `youth_time`=? AND `youth_end_time`=? AND `youth_description`=?");
		$stmt->execute(array($dateToWrite,$startTimeToWrite,$endTimeToWrite,$description));
		$row=$stmt->fetch(PDO::FETCH_LAZY);
		if ($row['COUNT(*)']<=0)
		{
			$stmt=$pdo->prepare("INSERT INTO `youth_club` SET `youth_date`=?, `youth_time`=?, `youth_end_time`=?, `youth_place`=?, `youth_description`=?");
	...
	echo '<a href="./loadYouthFromFile.php">Загрузить ещё события</a><br>';
?>
