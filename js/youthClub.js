$(function()
{
			...
			success: function(response)
			{
				...
				if (response.youth)
				{
					...
					for (var i=0;i<length;i++)
					{
						...
						row+=
						'<div class="row" data-id="'+youth[i].id+'">'+
							'<div class="col-sm-1 col-md-1 col-sm-1 col-xs-1 id">'+youth[i].id+'</div>'+
							'<div class="col-sm-2 col-md-2 col-sm-2 col-xs-2 date"><input type="text" data-id="'+youth[i].id+'" value="'+youth[i].date+'" class="date"></div>'+
							'<div class="col-sm-1 col-md-1 col-sm-1 col-xs-1 time"><input type="text" data-id="'+youth[i].id+'" value="'+youth[i].start_time+'" class="time"></div>'+
							'<div class="col-sm-1 col-md-1 col-sm-1 col-xs-1 end_time"><input type="text" data-id="'+youth[i].id+'" value="'+youth[i].end_time+'" class="end_time"></div>'+
							'<div class="col-sm-2 col-md-2 col-sm-2 col-xs-2 description"><textarea cols="18" class="descr" data-id="'+youth[i].id+'">'+youth[i].description+'</textarea></div>'+
							'<div class="col-sm-1 col-md-1 col-sm-1 col-xs-1 cancel"><input class="cancel" data-id="'+youth[i].id+'" type="checkbox" '+checkedAttr+'></div>'+
							'<div class="col-sm-2 col-md-2 col-sm-2 col-xs-2 place"><input type="text" data-id="'+youth[i].id+'" value="'+youth[i].place+'" class="place"></div>'+
							'<div class="col-sm-2 col-md-2 col-sm-2 col-xs-2 actions">'+
							  '<input class="save" type="button" data-id="'+youth[i].id+'" value="Сохранить">'+
							  '<input class="delete" type="button" data-id="'+youth[i].id+'" value="Удалить">'+
							'</div>'+
						'</div>';
					}
						$("#youth-list").html(row);
	...
	$('body').on("click","#refreshYouth",function()
	{	
		$.ajax(
		{
			type: "GET",
	...
	$('body').on("click","#newYouth",function()
	{
														...
														if (end_seconds>=0 && end_seconds<=59)
														{
															...
															if (startTimestamp.getTime()<=endTimestamp.getTime())
															{
																...
																$.ajax(
																{
																	type: "POST",
																	data:dataToSend,
																	url: "../php/createYouthManually.php",
																	dataType: "json",
																	success: function(response)
																	{
	...
	$('body').on("click",".cancel",function()
	{
		...
		var dataToSend={id:id,state:state};
		$.ajax(
		{
			type: "POST",
			data:dataToSend,
			url: "../php/saveCancelYouthClub.php",
			dataType: "json",
			success: function(response)
			{
				alert("Запись изменена");
