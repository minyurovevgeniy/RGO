...
//	include("./php/loadData.php");
	...
	$backUrl='<a href="loadEventsFromFile.php">Попробуйте ещё раз</a><br>';
	...
	{
		$stmt=$pdo->prepare("TRUNCATE `events`");
		$stmt->execute();
	}
	...
	for($rowNumber=$MIN_ROW;$rowNumber<=$MAX_ROW;$rowNumber++)
	{
		...
		$stmt=$pdo->prepare("SELECT COUNT(*) FROM `events` WHERE `event_date`=? AND `event_time`=? AND `event_end_time`=?");
		$stmt->execute(array($dateToWrite,$startTimeToWrite,$endTimeToWrite));
		$row=$stmt->fetch(PDO::FETCH_LAZY);
		if ($row['COUNT(*)']<=0)
		{
			$stmt=$pdo->prepare("INSERT INTO `events` SET `event_date`=?, `event_time`=?, `event_end_time`=?, `event_place`=?, `event_description`=?");
	...
	echo '<a href="./loadEventsFromFile.php">Загрузить ещё события</a><br>';
?>
